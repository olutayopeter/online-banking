...
public class FeesMntDetail {
	...
	public void init(FeesMnt feesMnt){
		setDirty(false);
		setFeesMnt(feesMnt);
		model = new ArrayList();
		dt = new Date();
	}
	public FeesMntDetail() {
		create();
		model = new ArrayList();
	}
	...
	public String duplicate(){
		FeesMnt feesMnt = (FeesMnt)getFeesMnt().clone();
		feesMnt.setId(null);
		setFeesMnt(feesMnt);
		setCreating(true);
		FacesUtils.addInfoMessage("duplication_successful");

		addWebAuditLogEntry("Duplicate", new StringBuilder("Button Label: Duplicate").append(". Previous Content: ID = ").append(getFeesMnt().getId()).append(". Values: ").append(getFeesMnt().toString()).toString());
	...
	private void addWebAuditLogEntry(String actionPerformed, String actionDetail){
		if(null != userManagerUtil){
	        ...
	        String uri = "/FeesMntDetailED.xhtml";
	...
	public String next(){
		...
		int index = lst.indexOf(getFeesMnt());
		...
		// Now move to record
		index++;
		FeesMnt feesMnt = (FeesMnt)lst.get(index);
		setFeesMnt(feesMnt);
		setCreating(false);
	...
	public String list(){
		FeesMntList feesMntList = null;
		try {
			feesMntList = FacesUtils.findBean("feesMntList", FeesMntList.class);
			feesMntList.setList(getModel());
			feesMntList.setFirstRowIndex(getFirstRowIndex());
		} catch (Throwable e) {
			e.printStackTrace();
		}
		return "/views/FeesMntListED";
	}
	...
	public String save(){
		...
		if(isCreating()){
			...
			addWebAuditLogEntry("Save", new StringBuilder("Button Label: Save").append(". New Content: ID = ").append(getFeesMnt().getId()).append(". Values: ").append(getFeesMnt().toString()).toString());

		}
		else {
			dao.update(getFeesMnt());

			addWebAuditLogEntry("Update", new StringBuilder("Button Label: Save").append(". New Content: ID = ").append(getFeesMnt().getId()).append(". Values: ").append(getFeesMnt().toString()).toString());
	...
	public String delete(){
		...
		if(isCreating()){
			...
			FeesMnt feesMnt = getFeesMnt();
			FeesMnt previousFeesMnt = (FeesMnt)getFeesMnt().clone();
			List lst = model;
			int rowCount = lst.size();
			int lastIndex = rowCount - 1;
			int index = lst.indexOf(feesMnt);
			if(rowCount == 0 || index == -1){
				FacesUtils.addErrorMessage("no_row_selection");
				return null;
			}
	...
	private boolean validate(){
		...
		String confCode = getFeesMnt().getConfCode();
	...
	col = null;
	fieldValue = null;
	fieldValue = getFeesMnt().getConfCode();
	isBlankField = false;
	...
	if(isBlankField == false){
		...
		if(isCreating()){
			// Disallow duplicate FeesMnt confCode
			if(null != col && col.size() > 0){
					FacesUtils.addErrorMessage(null,"duplicate_value",new Object[]{": FeesMnt -> confCode already exists"});
					hasErrors = true;
			}
		}
		else {
			...
			if(null != col && col.size() > 1){
				...
				Object db_pk = getFeesMnt().getId();
				Object en_pk = null;
				FeesMnt tfeesMnt = (FeesMnt)col.iterator().next();
				en_pk = tfeesMnt.getId();
	 ...
	 * @return the FeesMnt
	 */
	public FeesMnt getFeesMnt() {
		return feesMnt;
	}
	/**
	 * @param feesMnt the FeesMnt to set
	 */
	public void setFeesMnt(FeesMnt feesMnt) {
		this.feesMnt = feesMnt;
	}
	 ...
	 * @return the model
	 */
	public List<FeesMnt> getModel() {
		return model;
	}
	/**
	 * @param model the model to set
	 */
	public void setModel(List<FeesMnt> model) {
		this.model = model;
	}
