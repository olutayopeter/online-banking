...
public class ProductDetail {
	...
	public String create(){
		...
		product.setId(new Long(System.currentTimeMillis()).longValue());
		setProduct(product);
		setCreating(true);
	...
	public String duplicate(){
		Product product = (Product)getProduct().clone();
		product.setId(null);
		setProduct(product);
		setCreating(true);
		FacesUtils.addInfoMessage("duplication_successful");

		addWebAuditLogEntry("Duplicate", new StringBuilder("Button Label: Duplicate").append(". Previous Content: ID = ").append(getProduct().getId()).append(". Values: ").append(getProduct().toString()).toString());
	...
	private void addWebAuditLogEntry(String actionPerformed, String actionDetail){
		if(null != userManagerUtil){
	        String actionSource = "PRODUCT";
	        String uri = "/ProductDetailED.xhtml";
	...
	public String next(){
		...
		int index = lst.indexOf(getProduct());
		...
		// Now move to record
		index++;
		Product product = (Product)lst.get(index);
		setProduct(product);
		setCreating(false);
	...
	public String list(){
		...
		try {
			productList = FacesUtils.findBean("productList", ProductList.class);
			productList.setList(getModel());
			productList.setFirstRowIndex(getFirstRowIndex());
		} catch (Throwable e) {
			e.printStackTrace();
		}
		return "/views/ProductListED";
	}
	...
	public String save(){
		...
		if(isCreating()){
			...
			addWebAuditLogEntry("Save", new StringBuilder("Button Label: Save").append(". New Content: ID = ").append(getProduct().getId()).append(". Values: ").append(getProduct().toString()).toString());

		}
		else {
			dao.update(getProduct());

			addWebAuditLogEntry("Update", new StringBuilder("Button Label: Save").append(". New Content: ID = ").append(getProduct().getId()).append(". Values: ").append(getProduct().toString()).toString());
	...
	public String delete(){
		...
		if(isCreating()){
			...
			Product previousProduct = (Product)getProduct().clone();
			List lst = model;
			int rowCount = lst.size();
			int lastIndex = rowCount - 1;
		...
		product.setId(new Long(System.currentTimeMillis()).longValue());
			...
			addWebAuditLogEntry("Delete", new StringBuilder("Button Label: Delete").append(". Deleted Content: ID = ").append(previousProduct.getId()).append(". Values: ").append(previousProduct.toString()).toString());
	...
	private boolean validate(){
		...
		if(description == null || description.trim().equals("")){
			FacesUtils.addErrorMessage(null,"blank_field",new Object[]{"Description"});
			hasErrors = true;
		}
